...
for alpha in alphas:
    for epsilon in epsilons:
        # Results folder
        FOLDER = f"hpc_jobs/fullness_parameters/wait_first/alpha_{str(alpha).replace('.', '_')}_epsilon_{str(epsilon).replace('.', '_')}/"
        os.makedirs(FOLDER, exist_ok=True)

        check_parameters(dvrp_file, alpha, epsilon, results_folder=FOLDER, waiting_strategy="wait_first")
