...
for starting_capacity in starting_capacities:
    for full_capacity_time in full_capacity_times:
        # Results folder
        FOLDER = f"hpc_jobs/reduce_capacity_parameters/wait_first/starting_capacity_{str(starting_capacity).replace('.', '_')}_full_capacity_time_{str(full_capacity_time).replace('.', '_')}/"
        os.makedirs(FOLDER, exist_ok=True)

        check_parameters_reduce_capacity(dvrp_file, starting_capacity, full_capacity_time, FOLDER, "wait_first")
