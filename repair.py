...
def cheapest_insertion(capacity, adj_matrix, demands, working_day, durations, routes, single):
    inserted = False
    best_insert = (0, 0)
    best_insert_cost = 0
    for i, dynamic_route in enumerate(routes):
        ...
        if capacity_constraint_route(capacity, demands, dynamic_route.full_route() + [single]):
            ...
            for j, customer in enumerate(route + [0]):
                ...
                if time_constraint_route(working_day, durations, adj_matrix, suggested_route):
...
def repair(cost, capacity, adj_matrix, demands, simulation_time, working_day, durations, safe_routes, dangerous_routes):
    ...
    while dangerous_routes:
        ...
        route = dynamic_route.route
        if check_constraints(capacity, demands, working_day, durations, adj_matrix, dynamic_route):
            ...
            if len(first_route) == 1 and not(dynamic_route.covered_route):
                insertion_cost, safe_routes = cheapest_insertion(capacity, adj_matrix, demands, working_day, durations, safe_routes, first_route[0])
                cost += insertion_cost
            elif check_constraints(capacity, demands, working_day, durations, adj_matrix, dynamic_first_route):
            ...
            if len(second_route) == 1:
                ...
                cost += insertion_cost
            elif check_constraints(capacity, demands, working_day, durations, adj_matrix, dynamic_second_route):
