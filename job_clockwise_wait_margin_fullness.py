...
# Results folder
FOLDER = f"hpc_jobs/clockwise_tests/wait_margin_{str(wait_margin).replace('.', '_')}_fullness_alpha_{str(alpha).replace('.', '_')}_epsilon_{str(epsilon).replace('.', '_')}/"
os.makedirs(FOLDER, exist_ok=True)

check_parameters(dvrp_file, alpha, epsilon, results_folder=FOLDER, route_orientation_strategy="clockwise", waiting_strategy="wait_first", wait_margin=wait_margin)
