...
def two_opt(adj_matrix, routes):
    ...
    for i, route in enumerate(routes):
        ...
        # iterate over all lengths of segment to invert
        for length in range(2, len(route)+1):
            # iterate over all starting points of segment to invert
            for start in range(len(route)-length+1):
                ...
                if evaluate_route(adj_matrix, new_route) < route_cost:
                    return routes[:i] + [new_route] + routes[i+1:]
    return []


def two_opt_star(adj_matrix, routes):
    ...
    for i, route1 in enumerate(routes):
        ...
        for x, route2 in enumerate(routes[i+1:]):
            ...
            # iterate over all starting points in route 1
            for start1 in range(len(route1)+1):
                # iterate over all starting points in route 2
                for start2 in range(len(route2)+1):
...
def local_search(adj_matrix, routes):
    ...
    # try 2-opt until no more improvement possible
    new_solution = current_solution
    while new_solution:
        current_solution = new_solution
        new_solution = two_opt(adj_matrix, current_solution)

    # try 2-opt* until no more improvement possible
    new_solution = current_solution
    while new_solution:
        current_solution = new_solution
        new_solution = two_opt_star(adj_matrix, current_solution)
